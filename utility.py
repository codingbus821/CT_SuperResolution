...
class checkpoint():
    ...
    def save(self, model, is_best=False):
        # if not os.path.exists(os.path.join(model.state_dict(), 'model-dncnn')):
        #     os.mkdir(os.path.join(model.state_dict(), 'model-dncnn'))
        if is_best:
            torch.save(model.state_dict(), self.get_path('model', 'model_best.pt')) ################
            torch.save(model.state_dict(), self.get_path('model', 'model_last.pt')) ################
        else:
            torch.save(model.state_dict(), self.get_path('model', 'model_last.pt')) ################
